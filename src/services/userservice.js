...
export async function Update({nome, sobrenome, email }) {
    ...
    try{
        const res = await api.put('/user', { nome, sobrenome, email }, { headers: {'Authorization': `Bearer ${token}`} });
...
export async function Create({nome, email, senha}) {
    ...
    try{
        ...
        const res = await api.post('/user', { nome, email, senha }, { headers: {'Authorization': `Bearer ${token}`} });
